...
class Seq:
    ...
    def replace(self, seq, pos):
        # 检查 batch_size 是否兼容
        if self.bs != 1 and seq.bs != 1 and self.bs != seq.bs:
            raise ValueError(
                f"Cannot broadcast sequences with batch sizes {self.bs} and {seq.bs}."
            )
        # 检查 pos 是否有效
        if pos < 0 or pos > self.mask.size(1):
            raise IndexError(f"Position {pos} is out of range for sequence of length {self.mask.size(1)}.")
        # 处理 mask
        mask_list = expand_for_broadcast_tensor([self.mask, seq.mask], dim=0)
        new_mask = mask_list[0].clone()
        new_mask[:, pos:pos + seq.mask.size(1)] = mask_list[1]
        self.mask = new_mask
        # 根据数据类型进行替换
        if self.dtype == "logits":
            logits_list = expand_for_broadcast_tensor([self.logits, seq.logits], dim=0)
            new_logits = logits_list[0].clone()
            new_logits[:, pos:pos + seq.logits.size(1)] = logits_list[1]
            self.logits = new_logits
        elif self.dtype == "probs":
            probs_list = expand_for_broadcast_tensor([self.probs, seq.probs], dim=0)
            new_probs = probs_list[0].clone()
            new_probs[:, pos:pos + seq.probs.size(1)] = probs_list[1]
            self.probs = new_probs
        elif self.dtype == "ids":
            ids_list = expand_for_broadcast_tensor([self.ids, seq.ids], dim=0)
            new_ids = ids_list[0].clone()
            new_ids[:, pos:pos + seq.ids.size(1)] = ids_list[1]
            self.ids = new_ids
        elif self.dtype == "text":
            raise NotImplementedError("Replacing text is not implemented.")
        else:
            raise RuntimeError("No data to replace.")
