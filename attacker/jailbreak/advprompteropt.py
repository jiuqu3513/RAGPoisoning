...
@torch.no_grad()
def advPrompterOpt(cfg, instruct, target, prompter, target_llm, record_stats=False,front_doc=None,back_doc=None):
    if cfg.verbose:
    ...
    for idx in range(cfg.train.q_params.max_new_tokens):
        ...
        next_dist_seq_prompter, next_dist_seq_basemodel = get_next_token_probabilities(
            cfg=cfg, instruct=instruct_rep, suffix=suffix_beams, prompter=prompter, front_doc=front_doc,back_doc=back_doc
        )
...
def get_next_token_probabilities(cfg, instruct, suffix, prompter,front_doc,back_doc,task):
    # get the next token probabilities from the prompter and the base model
    prompter_next = prompter.get_next_token(
        key="suffix", instruct=instruct, suffix=suffix, front_doc=front_doc,back_doc=back_doc,task=task
    )
    next_dist_seq_prompter = (
        prompter_next.response_dist.clone().detach()
    )  # (ebs, 1, vocab_size)
    prompter_next_basemodel = prompter.get_next_token(
        key="suffix", instruct=instruct, suffix=suffix, use_basemodel=True,front_doc=front_doc,back_doc=back_doc,task=task
    )
